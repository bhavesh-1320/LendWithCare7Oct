...
addToCartFL(){
    ...
    .then(result => {
        ...
        if(result[0].Id.length >=15 || result[0].Id.length>=18){
            ...
            this.flRecord[0].Funded__c = this.flRecord[0].Funded__c + this.selectedAmount;
    ...
    .then(result => {
        ...
        if(result[0].Id.length >=15 || result[0].Id.length>=18){
            ...
            if (childComponent) {
                ...
                //console.log('after posting to local storage ',typeof(localStorage.getItem('myArray') ) );
                childComponent.loanidfromparent = this.currentRecordId; 
    ...
    console.log('currentRecordItem after copying ', this.currentRecordId)*/
     ...
     handleDelete(event) {
        ...
        const matchingObjDataIndex = this.objdata.findIndex(
            objRecord => objRecord.TransactionId == itemId
        );
...
@wire(loanRecord)
    loadVal(pageValue) {
      ...
      //console.log('returned records');
        if (data) {
             ...
             this.FLId=data[0].Id;
             this.Loan_Title__c=data[0].Loan_Title__c;
             this.loc=(data[0].Borrower__r.Postcode__c ? data[0].Borrower__r.Postcode__c : '' )+ ' ' +data[0].Borrower__r.Country__c;
             this.Loan_Type__c=data[0].Loan_Type__c;
             this.Loan_Description__c=data[0].Loan_Description__c;
             this.Funded__c=data[0].Amount_Funded__c?data[0].Amount_Funded__c:0;
             this.Published_Amount_AUD__c=data[0].Published_Amount_AUD__c?.toFixed(2);
             //this.progress = (data[0].Funded__c / data[0].Published_Amount_AUD__c) * 100;
             this.progress = data[0].Funded__c;
             this.isFullyFunded = (data[0].Funded__c === data[0].Published_Amount_AUD__c.toFixed(2)) ? true:false;
            this.fundingOptions= this.getFundingOptions((parseFloat(data[0].Published_Amount_AUD__c).toFixed(2) - data[0].Funded__c.toFixed(2)))
                ...
                fundingOptions: this.getFundingOptions(parseFloat(loan.Published_Amount_AUD__c - (loan.Amount_Funded__c?loan.Amount_Funded__c:0)).toFixed(2)),
                ...
                console.log('fl loan as object ', this.flRecord[0]);
    ...
    getFundingOptions(item) {
        ...
        //for( const i in this.amountsFromSettings){
            ...
            if(this.amountsFromSettings[i] < item && this.amountsFromSettings[i] != ''){
                options.push({ label: `$${this.amountsFromSettings[i]}`, 
                value: Number(this.amountsFromSettings[i]).toFixed(2),
                selected: Number(this.amountsFromSettings[i]).toFixed(2) == 50.00, });
            }
        }
            options.push({ label: `$${item}`, value: Number(item).toFixed(2) });
        //}
...
allLoanRecords(allLoanValue) {
      ...
      //console.log('returned records', JSON.stringify(allLoanValue));
      ...
      const storedArray = JSON.parse(localStorage.getItem('myArray'));
        ...
        if (storedArray) {
            ...
            console.log('from view all loans wire ', storedArray);
        }
            ...
            if(storedArray){
                storedArray.forEach(storedRecord => {
                    ...
                    if (matchingObjDataRecordIndex !== -1) {
                        ...
                        updatedObjdata[matchingObjDataRecordIndex].Funded__c = storedRecord.Funded__c;
                        updatedObjdata[matchingObjDataRecordIndex].progress = storedRecord.progress;
                        
                        // Add more fields as needed
                    }
                });
                this.objdata = updatedObjdata;
            }
...
get backgroundImage() {
  ...
  connectedCallback() {
    ...
    .then(result => {
          ...
          console.log('this.amountsFromSettings from conn ', this.amountsFromSettings)
            this.PAGE_SIZE = 9;
      })
    this.getScreenSize();
    ...
    console.log('lloc');
    const storedArray = JSON.parse(localStorage.getItem('myArray'));
    console.log('lloc2 889 ',typeof(storedArray));
  ...
  renderedCallback(){
    ...
    if(progressBarElements){
        ...
        const progressValue = (progressBar.dataset.value >=98.70)?98.70:progressBar.dataset.value;//progressBar.dataset.value - 2:progressBar.dataset.value;
