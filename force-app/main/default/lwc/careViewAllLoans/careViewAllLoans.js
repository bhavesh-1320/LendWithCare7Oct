...
export default class CareAboutUs extends LightningElement {
    ...
    addToCart(event) {
        ...
        console.log('Number(this.selectedAmount) ',Number(this.selectedAmount))
        if(Number(this.selectedAmount) > 0){
    ...
    allLoanRecords(allLoanValue) {
        ...
        if (data) {
            console.log('returned records-- ', (data[0].Amount_Funded__c, '-- ', data[0].Expected_Fund_From_Cart__c));
            ...
            //console.log('after mapping ',JSON.stringify(this.objdata))
            var lIds = [];
